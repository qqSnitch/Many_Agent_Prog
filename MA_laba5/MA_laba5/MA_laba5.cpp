...
//{
//	int id;
//	vector<int>conections; //Связи по id между модулями
//	double diff; //Нагрузка модуля
//	bool complete = false; //Флаг завершенности
//	bool starting = false; //Является ли начальным модулем
//	bool taken = false;
//	bool current = false; //Явялется ли доступным для выполнения
//};
//struct Agent
//{
//	int id;
//	vector<int>connections; //Связи по id между агентами
//	bool working = false; //Флаг занятости агента
//	int target = -1; //id module над которым идет работа
//};
//
//vector<Module> M;
//vector<Agent> A;
// 
//void createTest() { // Вариант 1: линейый последовательный граф    1-2-3-4-5
//	random_device rd;
//	mt19937 gen(rd());
//	uniform_real_distribution<double> dist(0.0, 3.0);
//	m = 5, n = 3;
//	M.resize(m);
//	for (int i = 0; i < m; i++) { //Создаем граф модулей
//		M[i].id = i;
//		int k = 0;
//		if (i != m - 1) {
//			M[i].conections.push_back(i + 1);
//		}
//		if (i != 0) {
//			M[i].conections.push_back(i - 1);
//		}
...
//	M[0].current = true;
//
//	for (int i = 0; i < n; i++) { //Создаем граф Агентов
//		A.resize(n);
//		A[i].id = i;
//		for (int j = 0; j < n; j++) {
//			if (i != j) {
//				A[i].connections.push_back(j); //Все агенты соеденены между собой
//			}
//		}
//	}
//}
...
//	int k = 0;
//	for (int i = 0; i < m; i++) {
//		if (M[i].starting && k<A.size()) {
//			A[k].target = M[i].id; //Сначала назначаем агентам стартовые модули
//			A[k].working = true;
//			M[i].taken = true;
//			k++;
//		}
//	}
//	for (int i = 0; i < m; i++) {
//		if (!M[i].taken && k < A.size()) {
//			A[k].target = M[i].id; //Потом оставшиеся
//			A[k].working = true;
//			M[i].taken = true;
//			k++;
//		}
